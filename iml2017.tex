...
\newcommand{\conference}{IML Machine Learning Working Group Meeting}
\title{Applying Bayesian parameter estimation to relativistic heavy-ion collisions}
\author[J.\ E.\ Bernhard]{Jonah E.\ Bernhard}
...
\begin{frame}[plain,noframenumbering]
  \begin{tikzpicture}[remember picture, overlay]
    \node[anchor=south east] at (current page.south east) {\scriptsize
      \href{https://journals.aps.org/prc/abstract/10.1103/PhysRevC.94.024907}{PRC 94, 024907 (2016)}
      \quad
      \href{https://arxiv.org/abs/\arxivnum}{arXiv:\arxivnum\ [\arxivclass]}
    };
  \end{tikzpicture}
  \centering
  \setstretch{1.35}
...
\begin{frame}{The goal}
  \centering
  \tikz\node[box=1, text width=\textwidth] {
    Quantitatively estimate the properties of the hot and dense
    QCD matter created in ultra-relativistic heavy-ion collisions
  }; \\[1em]
  Want properties of: \\[.5ex]
  \parbox{.8\textwidth}{
    \begin{itemize}
      \setlength\itemsep{1ex}
      \item The quark-gluon plasma (QGP) medium, \\
        e.g.\ specific shear viscosity $\eta/s$, \\
        including temperature dependence $(\eta/s)(T)$
      \item The initial state that leads to QGP formation, \\
        e.g.\ how energy / entropy is created
    \end{itemize}
  }
\end{frame}
...
\begin{frame}{Input parameters}
  ...
  \begin{columns}
    ...
    \begin{itemize}
      ...
      \item {\color{emph}Entropy deposition $p$}
    ...
    \begin{itemize}
      \item {\color{emph}$\eta/s$ min and slope}
      \item $\zeta/s$ norm
...
\newcommand\link[2]{\textbf{#1} [\url{#2}]}

\begin{frame}{Machine learning + statistics software}
  \begin{itemize}
    \setlength{\itemsep}{1.5ex}
    \item Python machine learning tools: \link{scikit-learn}{http://scikit-learn.org}
    \item MCMC sampling: \link{emcee}{http://dan.iel.fm/emcee}
    \item Gaussian processes: \link{george}{http://dan.iel.fm/george}, also scikit-learn
    \item Latin-hypercube sampling: \link{R lhs}{https://cran.r-project.org/package=lhs}
  \end{itemize}
  \bigskip\centering
  My github: \url{https://github.com/jbernhard}
\end{frame}
